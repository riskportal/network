...
from risk.network.graph import calculate_edge_lengths, get_best_dimple_factor
...
class NetworkIO:
    def __init__(
        ...
        _log_initialization(compute_sphere, dimple_factor, include_edge_weight)
    ...
    def _process_graph(self, G):
        ...
        if self.dimple_factor is None and self.compute_sphere:
            self.dimple_factor = get_best_dimple_factor(
                G.copy(),
...
def _log_initialization(compute_sphere, dimple_factor, include_edge_weight):
    ...
    for_print_dimple_factor = (
        f"with a [yellow]dimple factor[/yellow] of [red]{dimple_factor}[/red]"
        if dimple_factor and compute_sphere
        else "without a [yellow]dimple factor[/yellow]"
    )
    ...
    print(
        f"[cyan]Treating the network as {for_print_sphere} {for_print_dimple_factor} {for_print_edge_weight}...[/cyan]"
    )
