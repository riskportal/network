...
def compute_neighborhood_score_by_sum_cython(
    ...
    np.ndarray[np.float64_t, ndim=2] annotation_matrix,
    ):
    cdef np.float64_t[:, :] neighborhood_score = np.dot(neighborhoods, annotation_matrix)
...
def compute_neighborhood_score_by_zscore_cython(
    ...
    cdef np.ndarray[np.float64_t, ndim=2] EXX = np.dot(A, np.power(B, 2)) / N_reshaped
    cdef np.ndarray[np.float64_t, ndim=2] EEX = M**2
    # Standard deviation
    cdef np.ndarray[np.float64_t, ndim=2] std = np.sqrt(EXX - EEX)
